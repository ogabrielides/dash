...
static UniValue quorum_list(const JSONRPCRequest& request)
{
    ...
    for (const auto& lp : llmq::CLLMQUtils::GetEnabledQuorumParams(pindexTip)) {
        const auto& llmq_params = lp.get();
        UniValue v(UniValue::VARR);

        auto quorums = llmq::quorumManager->ScanQuorums(llmq_params.type, pindexTip, count > -1 ? count : llmq_params.signingActiveQuorumCount);
        for (auto& q : quorums) {
...
static UniValue quorum_info(const JSONRPCRequest& request)
{
    ...
    if (!Params().GetConsensus().legacy_llmqs.count(llmqType)) {
...
static UniValue quorum_sigs_cmd(const JSONRPCRequest& request)
{
    ...
    if (cmd == "sign") {
        ...
        if (request.params[5].isNull() || (request.params[5].get_str().empty() && !request.params[6].isNull())) {
            ...
            int signOffset{llmq::GetLegacyLLMQParams(llmqType).dkgInterval};
